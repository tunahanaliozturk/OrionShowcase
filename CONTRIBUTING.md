# Contributing to OrionShowcase

Thanks for taking the time to look at this. OrionShowcase is a banking sample that integrates all six Moongazing.Orion packages end-to-end. The project is small and the bar for contributions is "does it make the package clearer, faster, or safer without expanding the public surface needlessly."

## Before you open a PR

OrionShowcase is a sample application, not a library, so the contribution bar is different from the rest of the Orion family. We accept:

- Bug fixes (build errors, broken endpoints, incorrect Mermaid diagrams)
- Documentation improvements
- Patches that demonstrate a NEW Orion package feature once that feature ships in the underlying package
- Smaller-and-clearer rewrites

We do not accept:

- New aggregates / endpoints / business features that do not demonstrate an Orion package
- Replacing existing dependencies with personal preferences (e.g., swapping MediatR for handlers, EF Core for Dapper). The stack choices are deliberate.
- Restructuring the Clean Architecture layout. The four-layer split is intentional.

For anything beyond a typo or a one-line fix, please open an issue first.
