...
- Reader-writer account locks (OrionLock 0.4). Balance-mutating operations (deposit, withdraw, transfer) take an EXCLUSIVE per-account hold via `ISharedExclusiveLock.AcquireExclusiveAsync`, while the balance read (GetBalance) takes a SHARED hold via `AcquireSharedAsync`. Multiple reads run concurrently, but a read never runs alongside a mutation of the same account. A transfer takes the exclusive holds on its two accounts in sorted id order to avoid deadlock, and existing transfer idempotency is unchanged. OrionLock 0.4 ships shared/exclusive semantics for the in-memory backend only, so the sample wires an in-memory provider; a distributed reader-writer provider fits production behind the same abstraction. See the deposit/withdraw/transfer handlers, [`GetAccountBalanceHandler.cs`](src/Moongazing.OrionShowcase.Application/Accounts/Queries/GetAccountBalance/GetAccountBalanceHandler.cs), and [`InMemorySharedExclusiveLockProvider.cs`](src/Moongazing.OrionShowcase.Infrastructure/Locking/InMemorySharedExclusiveLockProvider.cs).
